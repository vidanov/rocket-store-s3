...
/*============================================================================*\
  ...
  This implementation has limited functionality at the moment, but it uses AWS S3 as a DB storage.
  ...
  +------------------+------------------+------------------------+
  | storage domain   |  database prefix |  directory             |
  +------------------+------------------+------------------------+
  | collection       |  table cluster   |  sub directory         |
  +------------------+------------------+------------------------+
  | key              |  key             |  file name             |
  +------------------+------------------+------------------------+
  | record           |  row             |  file contents         |
  +------------------+------------------+------------------------+
...
rocketstore.post = async (collection, key, record) => {
    try {
        ...
        s3 = new S3(rocketstore.credentials);
        var params = {
            Body: JSON.stringify(record),
            Bucket: rocketstore.data_storage_area,
            Key: `${rocketstore.data_storage_domain}/${collection}/${key}`
        };
...
rocketstore.get = async (collection, key) => {
    key = fileNameWash("" + (key || "")).replace(/[*]{2,}/g, '*');
    s3 = new S3(rocketstore.credentials);
    var params = {
        Bucket: rocketstore.data_storage_area,
        Key: `${rocketstore.data_storage_domain}/${collection}/${key}`
...
rocketstore.delete = async (collection = '', key = '') => {
    s3 = new S3(rocketstore.credentials);
    if (collection == '') {
        await emptyS3Directory(rocketstore.data_storage_area, `${rocketstore.data_storage_domain}`);
        console.log('all')
    } else {
        await emptyS3Directory(rocketstore.data_storage_area, `${rocketstore.data_storage_domain}/${collection}/${key}`);
    }
    return;
}
...
rocketstore.options = async (set_option) => {
    ...
    if (typeof set_option.data_storage_domain === "string") {
        rocketstore.data_storage_domain = set_option.data_storage_domain;
    } else {
        rocketstore.data_storage_domain = 'rocketstore';
    }
